...
import * as SDK from "@foundry-for-tunes/sdk";
import { QueryClient, useQuery, useQueryClient } from "@tanstack/react-query";
import { $ } from "../client";
import { openSnackbar } from "../snackbar";
import { queryClient } from "../query-client";

export function useSongsResult() {
  const { user } = useUser();
  return useQuery({
    queryKey: ["songs"],
    queryFn: async () => {
...
function patchSongImpl(
  ...
  const songsCopy = songs.map((song) => {
    if (song.id !== update.id) return song;

    for (const key of Object.keys(update)) {
      if (key === "id") continue;
      const previousValue = (song as any)[key];
      if (previousValue === (update as any)[key]) continue;
      updated = true;
  ...
  client.setQueryData(["songs"], songsCopy);
...
function patchSong(
  ...
  return () => patchSongImpl(client, previousValues);
}
